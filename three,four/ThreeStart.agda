...
module three,four.ThreeStart where
...
<=-antisym (osuc x) (osuc x1) = ap  suc  (<=-antisym x x1)
...
--                                   (m +N n *N m)  *N k == m *N k   +N   n   *N m *N k
--                                   (n +N m)       *N k == n *N k   +N   m   *N k
...
length-+L-distrib {A} [] ys = refl
length-+L-distrib {A} (x ,- xs) ys = ap suc (length-+L-distrib xs ys)

vecToList : {A : Set} {n : Nat} -> Vec A n -> List A
vecToList [] = [] 
vecToList (x ,- x1) = vecToList x1
...
-- reverse-+L-distrib (x ,- xs) ys rewrite reverse-+L-distrib xs ys = +L-assoc {!reverse ys  !} {!reverse xs  !} {!  (x ,- []) !}


-- might need reverse-+L-distrib here
-- reverse-involut : {A : Set} (xs : List A) -> reverse (reverse xs) == xs
-- reverse-involut [] = refl
-- reverse-involut (x ,- xs)   rewrite reverse-+L-distrib (reverse xs) (x ,- [])   = ap (x ,-_) (reverse-involut xs)
