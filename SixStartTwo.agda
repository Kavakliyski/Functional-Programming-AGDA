...
ext {gamma1} {delta1} {sigma} x {.sigma} Z = Z
ext {gamma1} {delta1} {sigma} x {tau} (S y) = S (x y)
...
extSubstitution x Z = var Z
extSubstitution x (S Z) = {!   !}
extSubstitution x (S (S y)) = extSubstitution (λ z -> x (S z)) (S y)
-}
...
substitute x (var x1) = x x1
substitute x (app y y1) = app (substitute x y) (substitute x y1)
substitute x (lam y) = {!     !}
...
reduceSubst (var x) y = app (lam (var y)) (var x)
reduceSubst (app x x1) y = app (lam (var y)) (app x x1)
reduceSubst (lam x) y = app (lam (var y)) (lam x)
...
substitute1 :
  ...
  Lam gamma sigma ->
  Lam gamma tau
substitute1 x (var x1) = app (lam x) (var x1)
substitute1 x (app y y1) = app (lam x) (app y y1)
substitute1 x (lam y) = app (lam x) (lam y)
...
insert tau gamma1 zero = gamma1
insert (base x1) gamma1 (suc x) = gamma1
insert (tau => tau1) gamma1 (suc x) = gamma1
{-
